...
void update_or_insert_counts(unordered_map<int, int> &counts, int key,
                             int update) {
...
int compute_decimal_value(unordered_map<int, int> &counts,
                          function<bool(int)> comparator) {
  int value = 0;
...
string compute_part_2_ratings(vector<string> &numbers,
  ...
  while (lo < (hi - 1)) {
    ...
    int half = (hi - lo) / 2;
    // cout << "count: " << count << " half: " << half << " curr: " << (curr -
    // lo) << endl;
...
int part1(ifstream &&ifs) {
  ...
  int gamma =
      compute_decimal_value(counts, [](int count) { return count > 0; });
  int epsilon =
      compute_decimal_value(counts, [](int count) { return count < 0; });
...
int part2(ifstream &&ifs) {
  ...
  string oxygen = compute_part_2_ratings(
      numbers, [](int count, int half) { return count <= half; });
  string co2 = compute_part_2_ratings(
      numbers, [](int count, int half) { return count > half; });
